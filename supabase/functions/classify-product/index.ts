...
serve(async (req) => {
  ...
  try {
    const {
      title, description, materials, dimensions, weight,
      originCountry, destinationCountry, imageUrl, productUrl,
      shipmentId, productId, workspaceId
    } = await req.json();

    const SUPABASE_URL = Deno.env.get("SUPABASE_URL")!;
    const SUPABASE_SERVICE_ROLE_KEY = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!;

    // Auth check — optional, used for audit logging
...
- What documents/certifications are needed to claim each preferential rate`;
      ...
      input_schema: {
        ...
        properties: {
          ...
          additionalDuties: {
            type: "array",
            items: { type: "string" },
            description: "Anti-dumping, countervailing, Section 301, safeguard duties",
          },
    ...
    const { data: classRecord, error: insertError } = await adminClient
      ...
      .insert({
        ...
        confidence: (result.confidence as number) / 100,
        accepted_code: result.primaryCode,
        status: "classified",
        ai_model_version: Deno.env.get("ANTHROPIC_API_KEY") ? "claude-sonnet-4-5" : "google/gemini-2.5-flash",
